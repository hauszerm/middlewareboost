...
{
    class MessageHandler<TData>
    {
        public void ResetDataHolder(AsyncUserToken<TData> token)
        {
            token.theDataHolder = default(TCPMessage<TData>);
        }

        public bool HandleMessage(SocketAsyncEventArgs receiveSendEventArgs, AsyncUserToken<TData> receiveSendToken, Int32 remainingBytesToProcess)
        {
            ...
            if (receiveSendToken.receivedMessageBytesDoneCount == 0)
            {
                ...
                TCPMessage<TData> msg = new TCPMessage<TData>() {
                    Client = null, //TCPClient Object not available here
                    RawData = new Byte[receiveSendToken.lengthOfCurrentIncomingMessage],
                };

                receiveSendToken.theDataHolder = msg;
            }


            // Write/append the bytes received to the byte array in the 
            // DataHolder object that we are using to store our data.
            Buffer.BlockCopy(receiveSendEventArgs.Buffer, receiveSendToken.receiveMessageOffset, receiveSendToken.theDataHolder.RawData, receiveSendToken.receivedMessageBytesDoneCount, remainingBytesToProcess);
            ...
            if (remainingBytesToProcess + receiveSendToken.receivedMessageBytesDoneCount == receiveSendToken.lengthOfCurrentIncomingMessage)
            {
                ...
                // Write/append the bytes received to the byte array in the 
                // DataHolder object that we are using to store our data.
                //Buffer.BlockCopy(receiveSendEventArgs.Buffer, receiveSendToken.receiveMessageOffset, receiveSendToken.theDataHolder.RawData, receiveSendToken.receivedMessageBytesDoneCount, remainingBytesToProcess); //moved outside the if
